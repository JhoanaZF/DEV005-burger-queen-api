...
describe("POST /products", () => {
    it("should fail with 401 when no auth", () => fetch("/products", { method: "POST" }).then((resp) => expect(resp.status).toBe(401)));
    ...
    it("should fail with 400 when bad props", () => fetchAsAdmin("/products", { method: "POST" }).then((resp) => expect(resp.status).toBe(400)));
...
describe("GET /products/:productid", () => {
    it("should fail with 404 when not found", () => fetchAsTestUser("/products/notarealproduct").then((resp) => expect(resp.status).toBe(404)));
...
describe("PUT /products/:productid", () => {
    it("should fail with 401 when no auth", () => fetch("/products/xxx", { method: "PUT" }).then((resp) => expect(resp.status).toBe(401)));
    ...
    it("should fail with 404 when admin and not found", () =>
        fetchAsAdmin("/products/65796d5235ab199be5fce6f8", {
            method: "PUT",
            body: { price: 1 },
...
describe("DELETE /products/:productid", () => {
    it("should fail with 401 when no auth", () => fetch("/products/xxx", { method: "DELETE" }).then((resp) => expect(resp.status).toBe(401)));
    ...
    it("should fail with 404 when admin and not found", () => fetchAsAdmin("/products/67996424d2e2dc19ac21736a", { method: "DELETE" }).then((resp) => expect(resp.status).toBe(404)));
