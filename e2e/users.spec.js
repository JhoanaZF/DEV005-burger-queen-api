const { fetch, fetchAsTestUser, fetchAsAdmin, fetchWithAuth, config, url, qs, adminUserCredentials, baseUrl } = process;
...
const parseLinkHeader = (str) => {
    ...
    return str.split(",").reduce((memo, item) => {
    ...
    it("should get users with pagination", () =>
        fetchAsAdmin("/users?limit=1")
            ...
            .then(({ headers, json }) => {
                ...
                return fetchAsAdmin("/users?limit=1");
            })
            .then((resp) => {
                expect(resp.status).toBe(200);
                return resp.json().then((json) => ({ headers: resp.headers, json }));
...
describe("GET /users/:uid", () => {
    it("should fail with 401 when no auth", () => fetch("/users/foo@bar.baz").then((resp) => expect(resp.status).toBe(401)));

    it("should fail with 403 when not owner nor admin", () => fetchAsTestUser(`/users/${config.adminEmail}`).then((resp) => expect(resp.status).toBe(403)));

    it("should fail with 404 when admin and not found", () => fetchAsAdmin("/users/abc@def.ghi").then((resp) => expect(resp.status).toBe(404)));
...
describe("POST /users", () => {
    it("should respond with 400 when email and password missing", () => fetchAsAdmin("/users", { method: "POST" }).then((resp) => expect(resp.status).toBe(400)));

    it("should respond with 400 when email is missing", () => fetchAsAdmin("/users", { method: "POST", body: { email: "", password: "xxxx" } }).then((resp) => expect(resp.status).toBe(400)));

    it("should respond with 400 when password is missing", () => fetchAsAdmin("/users", { method: "POST", body: { email: "foo@bar.baz" } }).then((resp) => expect(resp.status).toBe(400)));

    it("should fail with 400 when invalid email", () => fetchAsAdmin("/users", { method: "POST", body: { email: "failemail", password: "123456" } }).then((resp) => expect(resp.status).toBe(400)));

    it("should fail with 400 when invalid password", () => fetchAsAdmin("/users", { method: "POST", body: { email: "email@test.tes", password: "12" } }).then((resp) => expect(resp.status).toBe(400)));
...
describe("PUT /users/:uid", () => {
    it("should fail with 401 when no auth", () => fetch("/users/foo@bar.baz", { method: "PUT" }).then((resp) => expect(resp.status).toBe(401)));

    // it("should fail with 403 when not owner nor admin", () => fetchAsTestUser(`/users/${config.adminEmail}`, { method: "PUT" }).then((resp) => expect(resp.status).toBe(403)));

    it("should fail with 404 when admin and not found", () => fetchAsAdmin("/users/abc@def.gih", { method: "PUT" }).then((resp) => expect(resp.status).toBe(404)));

    it("should fail with 400 when no props to update", () => fetchAsTestUser("/users/test@test.test", { method: "PUT" }).then((resp) => expect(resp.status).toBe(400)));
...
describe("DELETE /users/:uid", () => {
    it("should fail with 401 when no auth", () => fetch("/users/foo@bar.baz", { method: "DELETE" }).then((resp) => expect(resp.status).toBe(401)));

    it("should fail with 403 when not owner nor admin", () => fetchAsTestUser(`/users/${config.adminEmail}`, { method: "DELETE" }).then((resp) => expect(resp.status).toBe(403)));

    it("should fail with 404 when admin and not found", () => fetchAsAdmin("/users/abc@def.ghi", { method: "DELETE" }).then((resp) => expect(resp.status).toBe(404)));
    ...
    it("should delete own user", () => {
        ...
        return fetchAsAdmin("/users", { method: "POST", body: credentials })
            ...
            .then(({ token }) =>
                fetchWithAuth(adminUserCredentials)(`/users/${credentials.email}`, {
                    method: "DELETE",
                })
            )
            .then((resp) => expect(resp.status).toBe(200))
